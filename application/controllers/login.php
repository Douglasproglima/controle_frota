...
class Login extends CI_Controller {
	...
	public function autentica(){
		...
		{						
			...
			if(iniciar_sessao($email, $senha)){
				...
				$data["erro"] = "Usuário e/ou senha inválidos";
				$this->load->view('topo');
	...
	public function recuperar_senha(){
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			if($usuarioInstance){
				...
				$data["erro"] = "Email não cadastrado";
				$this->load->view('topo');
	...
	public function _enviarSenhaPorEmail($usuarioInstance, $novaSenha){
		$config = Array(
		    ...
		    'smtp_port' => 465,
		    'smtp_user' => 'douglasproglimateste@gmail.com',
		    'smtp_pass' => 'dga,@!2612',
		    'mailtype'  => 'text', 
		    'charset'   => 'iso-8859-1'
		);
		...
		$this->email->from('douglasproglimateste@gmail.com', 'Douglas Lima');
		...
		$mensagem += "\n\rSua senha antiga não é mais válida.";
		$mensagem += "\n\rSenha : " . $novaSenha;
	...
	public function _mandaEmail($usuarioInstance, $novaSenha){
		...
		$mensagem = 'Sua nova senha foi gerada em nosso site em ' . date("d/m/Y h:i:s") . ".";
		$mensagem += "\nSua senha antiga não é mais válida.";
		$mensagem += "\nSenha : " . $novaSenha;
